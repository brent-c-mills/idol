...
completion() {
    echo "user_hash_ubuntu.sh has completed for idol "${IDOL_NAME} | tee -a ${CURRENT_LOG};
...
generate_user_hash_bats() {
    echo "Generating user Hash Test for "${IDOL_NAME} >> ${CURRENT_LOG};
...
handoff() {
    echo "user_hash_ubuntu.sh has been kicked off by idol_create.sh..." | tee -a ${CURRENT_LOG};
    echo "user_hash_ubuntu.sh is initiating user hash BATS creation..." | tee -a ${CURRENT_LOG};

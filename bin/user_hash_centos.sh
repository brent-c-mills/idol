...
completion() {
    echo "user_hash_centos.sh has completed for idol "${IDOL_NAME} | tee -a ${CURRENT_LOG};
...
handoff() {
    echo "user_hash_centos.sh has been kicked off by idol_create.sh..." | tee -a ${CURRENT_LOG};
    echo "user_hash_centos.sh is initiating user hash BATS creation..." | tee -a ${CURRENT_LOG};

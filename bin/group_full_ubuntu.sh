...
completion() {
    echo "group_full_ubuntu.sh has completed for idol "${IDOL_NAME} | tee -a ${CURRENT_LOG};
...
generate_group_bats() {
    while IFS=, read -r group; do
        ...
        echo "Adding group_full test for "${group} >> ${CURRENT_LOG};
...
handoff() {
    echo "group_full_ubuntu.sh has been kicked off by idol_create.sh..." | tee -a ${CURRENT_LOG};
    echo "group_full_ubuntu.sh is initiating full group BATS creation..." | tee -a ${CURRENT_LOG};

...
completion() {
    echo "user_full_ubuntu.sh has completed for idol "${IDOL_NAME} | tee -a ${CURRENT_LOG};
...
handoff() {
    echo "user_full_ubuntu.sh has been kicked off by idol_create.sh..." | tee -a ${CURRENT_LOG};
    echo "user_full_ubuntu.sh is initiating full user BATS creation..." | tee -a ${CURRENT_LOG};
    echo "idol name.................."${IDOL_NAME} | tee -a ${CURRENT_LOG};
    echo "users to be processed..."$(dpkg --list | awk '{ print $2 }' | wc -l) | tee -a ${CURRENT_LOG};

...
completion() {
    echo "chef_hash_darwin.sh has completed for idol "${IDOL_NAME} | tee -a ${LOG_OUT};
...
handoff() {
    echo "chef_hash_darwin.sh has been kicked off by idol_create.sh..." | tee -a ${LOG_OUT};
    echo "chef_hash_darwin.sh is initiating chef hash BATS creation..." | tee -a ${LOG_OUT};

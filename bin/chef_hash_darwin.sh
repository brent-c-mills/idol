...
completion() {
    echo "chef_hash_darwin.sh has completed for idol "${IDOL_NAME} | tee -a ${CURRENT_LOG};
...
generate_chef_hash() {
	...
	local hashgolden=$(md5 /tmp/chef_hash.txt | awk '{ print $4 }');
	echo $hashgolden;
...
handoff() {
    echo "chef_hash_darwin.sh has been kicked off by idol_create.sh..." | tee -a ${CURRENT_LOG};
    echo "chef_hash_darwin.sh is initiating chef hash BATS creation..." | tee -a ${CURRENT_LOG};

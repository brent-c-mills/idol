...
completion() {
    echo "environment_full_ubuntu.sh has completed for idol "$IDOL_NAME | tee -a $LOG_OUT;
    exit 0;
}
...
handoff() {
    echo "environment_full_ubuntu.sh has been kicked off by idol_create.sh..." | tee -a $LOG_OUT;
    echo "environment_full_ubuntu.sh is initiating full environment BATS creation..." | tee -a $LOG_OUT;

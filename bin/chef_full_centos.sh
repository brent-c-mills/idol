...
completion() {
    echo "chef_full_centos.sh has completed for idol "${IDOL_NAME} | tee -a ${CURRENT_LOG};
...
handoff() {
    echo "chef_full_centos.sh has been kicked off by idol_create.sh..." | tee -a ${CURRENT_LOG};
    echo "chef_full_centos.sh is initiating full chef BATS creation..." | tee -a ${CURRENT_LOG};

...
completion() {
    echo "chef_full_centos.sh has completed for idol "${IDOL_NAME} | tee -a ${LOG_OUT};
...
handoff() {
    echo "chef_full_centos.sh has been kicked off by idol_create.sh..." | tee -a ${LOG_OUT};
    echo "chef_full_centos.sh is initiating full chef BATS creation..." | tee -a ${LOG_OUT};

...
completion() {
    echo "group_full_darwin.sh has completed for idol "${IDOL_NAME} | tee -a ${CURRENT_LOG};
...
handoff() {
    echo "group_full_darwin.sh has been kicked off by idol_create.sh..." | tee -a ${CURRENT_LOG};
    echo "group_full_darwin.sh is initiating full group BATS creation..." | tee -a ${CURRENT_LOG};
